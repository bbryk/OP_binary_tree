...
class LinkedBST(AbstractCollection):
    ...
    def rebalance(self):
        ...
        def rebalance1(d: dict, tree: list, level=0):
            if level not in d.keys():
                d[level] = []
            if len(tree) == 1:
                d[level].append(tree[0])
                return None
            mid_index = len(tree) // 2

            # print(f'lvl: {level}')
            # print(f'mid: {mid_index}')
            # print(f'el: {tree[mid_index]}')
            d[level].append(tree[mid_index])
            tree1 = tree[:mid_index]
            rebalance1(d, tree1, level=level + 1)
            tree2 = tree[mid_index:]
            rebalance1(d, tree2, level=level + 1)
            return d
